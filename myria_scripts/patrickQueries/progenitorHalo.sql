...
store(progenitors, patrick:test:progen);

progenitors = scan(patrick:test:progen);
--gets only halos in tree
haloTable1 = [from scan(patrick:test:haloTable) h, edges e where h.nowGroup = e.nowGroup and h.grpID = e.currentGroup and h.timeStep = e.currentTime emit h.*];
haloTable2 = [from scan(patrick:test:haloTable) h, edges e where h.nowGroup = e.nowGroup and h.grpID = e.nextGroup and h.timeStep = e.currentTime+1 emit h.*];
...
store(final, patrick:test:haloTableProg);
