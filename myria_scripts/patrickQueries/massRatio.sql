...
haloTable = scan(patrick:test:haloTableProg);
rankedEdges = [from scan(patrick:test:edgesWithMassSort) as e emit e.nowGroup, e.currentTime,  RunningRank(e.currentGroup) as splitOrder, e.currentGroup, e.nextGroup, e.nextGroupMass];
...
--newHaloTable = newHaloTable + scan(patrick:test:haloTableComplete);
store(newHaloTable, patrick:test:haloTableComplete);

--WAY TWO

-- haloTable = scan(patrick:test:haloTableProg);
-- edges = scan(patrick:test:edgesTree);
...
-- store(firstMaxMass, patrick:test:firstMaxMassDec16);
...
-- store(newHaloTable, patrick:test:haloTableProgDec16);
